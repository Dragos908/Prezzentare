...
class _SlideIndexUpdated extends ControlEvent {
  final int idx;
  _SlideIndexUpdated(this.idx);
}

class _TouchUpdated extends ControlEvent {
  final bool val;
  _TouchUpdated(this.val);
}

class _VolumeUpdated extends ControlEvent {
  final double vol;
  _VolumeUpdated(this.vol);
}

class _SlidesUpdated extends ControlEvent {
  final List<SlideModel> slides;
  _SlidesUpdated(this.slides);
}

class _TimerRunningUpdated extends ControlEvent {
  final bool running;
  _TimerRunningUpdated(this.running);
}

class _TimerBaseUpdated extends ControlEvent {
  final int base;
  _TimerBaseUpdated(this.base);
}

class _TimerStartUpdated extends ControlEvent {
  final int start;
  _TimerStartUpdated(this.start);
}

class _SlideTimersUpdated extends ControlEvent {
  final Map<int, SlideTimerData> slideTimers;
  _SlideTimersUpdated(this.slideTimers);
}

class _IframePageUpdated extends ControlEvent {
  final int page;
  _IframePageUpdated(this.page);
}

class _OverlayUpdated extends ControlEvent {
  final bool enabled;
  _OverlayUpdated(this.enabled);
}

class _PointerUpdated extends ControlEvent {
  final double x, y;
  final bool active;
  _PointerUpdated(this.x, this.y, this.active);
}
...
class ControlBloc extends Bloc<ControlEvent, PresentationState> {
  ...
  ControlBloc(this._fb) : super(const PresentationState()) {
    ...
    on<_TouchUpdated>((e, emit) =>
        emit(state.copyWith(touchEnabled: e.val)));

    on<_VolumeUpdated>((e, emit) =>
        emit(state.copyWith(volume: e.vol)));

    on<_SlidesUpdated>((e, emit) =>
        emit(state.copyWith(slides: e.slides)));

    on<_TimerRunningUpdated>((e, emit) =>
        emit(state.copyWith(timerRunning: e.running)));

    on<_TimerBaseUpdated>((e, emit) =>
        emit(state.copyWith(timerBase: e.base)));

    on<_TimerStartUpdated>((e, emit) =>
        emit(state.copyWith(timerStart: e.start)));

    on<_SlideTimersUpdated>((e, emit) =>
        emit(state.copyWith(slideTimers: e.slideTimers)));

    on<_IframePageUpdated>((e, emit) =>
        emit(state.copyWith(iframePageIndex: e.page)));

    on<_OverlayUpdated>((e, emit) =>
        emit(state.copyWith(overlayEnabled: e.enabled)));
    ...
    _touchSub = _fb.touchEnabledStream
        .listen((v) => add(_TouchUpdated(v)));

    _volumeSub = _fb.volumeStream
        .listen((v) => add(_VolumeUpdated(v)));

    _slidesSub = _fb.slidesStream
        .listen((s) => add(_SlidesUpdated(s)));

    _timerRunningSub = _fb.timerRunningStream
        .listen((v) => add(_TimerRunningUpdated(v)));

    _timerBaseSub = _fb.timerBaseStream
        .listen((v) => add(_TimerBaseUpdated(v)));

    _timerStartSub = _fb.timerStartStream
        .listen((v) => add(_TimerStartUpdated(v)));

    _slideTimersSub = _fb.slideTimersStream
        .listen((m) => add(_SlideTimersUpdated(m)));

    _iframePageSub = _fb.iframePageIndexStream
        .listen((p) => add(_IframePageUpdated(p)));

    _overlaySub = _fb.overlayEnabledStream
        .listen((v) => add(_OverlayUpdated(v)));
  ...
  Future<void> _onToggleTouch(
    ...
    await _fb.setTouchEnabled(!state.touchEnabled);
  }

  Future<void> _onSetVolume(
      SetVolumeEvent e, Emitter<PresentationState> emit) async {
    await _fb.setVolume(e.vol);
  }
